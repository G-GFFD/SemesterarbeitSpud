...
//For debugging, reroute all SPUD packets to this address
...
void *tcptospud(void* argument)
{
	...
	while(1)
	{
		...
		if((size = recvmsg(sock_fd, &msg, 0)) != -1)
		{
			...
			//Argument is where the TCP header starts in the message buffer
			tcph = extracttcph(message+(iph->ihl*4));

			
			int tcpdatalenght = (int)(ntohs(iph->tot_len))-((int)(tcph->doff)+((int)iph->ihl))*4;
			data = extracttcpdata(message+(iph->ihl*4),tcpdatalenght,tcph);

			printf("Sequence Nr. of TCP about to be sent %i \n", ntohl(tcph->seq));
			printf("Ack Nr. of TCP about to be sent %i \n", ntohl(tcph->ack));
			
			//Debug only, remove afterwards . . .
			printf("Current tcp checksum %i\n", tcph->check);
			updatetcpchecksum(tcph,iph,data);
			printf("Recalculated tcp checksum %i", tcph->check);
			//.....
